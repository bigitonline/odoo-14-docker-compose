...
class PayslipDetailsReport(models.AbstractModel):
    _name = 'report.wk_hr_payroll.report_payslipdetails'
    _description = 'Payslip Details Report'

    def get_details_by_rule_category(self, payslip_lines):
        PayslipLine = self.env['wk.hr.payslip.line']
        RuleCateg = self.env['wk.hr.salary.rule.category']
        ...
        if payslip_lines:
            self.env.cr.execute(
                """
                SELECT pl.id, pl.category_id, pl.slip_id FROM wk_hr_payslip_line as pl
                LEFT JOIN wk_hr_salary_rule_category AS rc on (pl.category_id = rc.id)
                WHERE pl.id in %s
    ...
    def _get_report_values(self, docids, data=None):
        payslips = self.env['wk.hr.payslip'].browse(docids)
        return {
            'doc_ids': docids,
