...
class ContributionRegisterReport(models.AbstractModel):
    _name = 'report.wk_hr_payroll.report_contributionregister'
    ...
    def _get_payslip_lines(self, register_ids, date_from, date_to):
        ...
        self.env.cr.execute(
            """
            SELECT pl.id from wk_hr_payslip_line as pl
            LEFT JOIN wk_hr_payslip AS hp on (pl.slip_id = hp.id)
        ...
        for line in self.env['wk.hr.payslip.line'].browse(line_ids):
            result.setdefault(line.register_id.id, self.env['wk.hr.payslip.line'])
    ...
    def _get_report_values(self, docids, data=None):
        ...
        contrib_registers = self.env['wk.hr.contribution.register'].browse(
            register_ids)
        ...
        return {
            ...
            'doc_model': 'wk.hr.contribution.register',
