...
from odoo.addons.wk_hr_payroll.tests.common import TestPayslipBase
...
class TestPayslipFlow(TestPayslipBase):
    ...
    def test_00_payslip_flow(self):
        ...
        richard_payslip = self.env['wk.hr.payslip'].create({
        ...
        context = {
            ...
            "active_ids": [self.ref("wk_hr_payroll.menu_department_tree")],
            "active_id": self.ref("wk_hr_payroll.menu_department_tree")
        }
        ...
        payslip_refund = self.env['wk.hr.payslip'].search([
        ...
        payslip_run = self.env['wk.hr.payslip.run'].create({
            'date_end': '2011-09-30',
        ...
        payslip_employee = self.env['wk.hr.payslip.employees'].create(
        ...
        self.env['wk.payslip.lines.contribution.register'].create({
            'date_from': '2011-09-30',
            'date_to': '2011-09-01'
        })

        # I print the payslip report
        data, data_format = self.env.ref('wk_hr_payroll.action_report_payslip').render(richard_payslip.ids)

        # I print the payslip details report
        data, data_format = self.env.ref('wk_hr_payroll.payslip_details_report').render(richard_payslip.ids)

        # I print the contribution register report
        context = {
            'model': 'wk.hr.contribution.register',
            'active_ids': [self.ref('wk_hr_payroll.hr_houserent_register')]
        }
        test_reports.try_report_action(self.env.cr, self.env.uid, 'action_wk_payslip_lines_contribution_register', context=context, our_module='hr_payroll')
