...
class WkHrSalaryRuleCategory(models.Model):
    _name = 'wk.hr.salary.rule.category'
    _description = 'Salary Rule Category'

    name = fields.Char(required=True, translate=True)
    code = fields.Char(required=True)
    parent_id = fields.Many2one('wk.hr.salary.rule.category', string='Parent',
        help="Linking a salary category to its parent is used only for the reporting purpose."
    )
    children_ids = fields.One2many('wk.hr.salary.rule.category', 'parent_id', string='Children')
...
class WkHrSalaryRule(models.Model):
    _name = 'wk.hr.salary.rule'
    _order = 'sequence, id'
    ...
    category_id = fields.Many2one('wk.hr.salary.rule.category', string='Category', required=True)
    ...
    appears_on_payslip = fields.Boolean(string='Appears on Payslip',
        default=True, help="Used to display the salary rule on payslip.")
    parent_rule_id = fields.Many2one('wk.hr.salary.rule', string='Parent Salary Rule', index=True)
    company_id = fields.Many2one('res.company', string='Company',
        default=lambda self: self.env.company)
    ...
    child_ids = fields.One2many('wk.hr.salary.rule', 'parent_rule_id',
                                string='Child Salary Rule', copy=True)
    register_id = fields.Many2one('wk.hr.contribution.register', string='Contribution Register',
        help="Eventual third party involved in the salary payment of the employees."
    )
    input_ids = fields.One2many('wk.hr.rule.input', 'input_id', string='Inputs', copy=True)
    note = fields.Text(string='Description')
    ...
    def _compute_rule(self, localdict):
        ...
        if self.amount_select == 'fix':
            ...
            try:
                ...
                raise UserError(
                    _('Wrong python code defined for salary rule %s (%s).') % (self.name, self.code))
    ...
    def _satisfy_condition(self, localdict):
        ...
        if self.condition_select == 'none':
            ...
            try:
                ...
                raise UserError(
                    _('Wrong range condition defined for salary rule %s (%s).') % (self.name, self.code))
        else:  # python code
            try:
                ...
                raise UserError(
                    _('Wrong python condition defined for salary rule %s (%s).') % (self.name, self.code))


class WkHrRuleInput(models.Model):
    _name = 'wk.hr.rule.input'
    _description = 'Salary Rule Input'

    name = fields.Char(string='Description', required=True)
    code = fields.Char(required=True,
                       help="The code that can be used in the salary rules")
    input_id = fields.Many2one('wk.hr.salary.rule', string='Salary Rule Input', required=True)
